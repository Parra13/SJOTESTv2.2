...
if(!empty($_POST['passwordAntiguo'])){
	$password=mysql_real_escape_string(trim(strip_tags(md5($_POST['passwordAntiguo']))));//obtenemos el password antiguo
	$select="SELECT pass FROM usuario WHERE userid='".$userActivo."' AND pass='".$password."'";//preparamos un select
	$querySelect=mysql_query($select,$conexion);//lanzamos la conexion
	
	if(!mysql_num_rows($querySelect)){//comprobamos que haya coincidencia
	...
	$passwordNuevo=mysql_real_escape_string(trim(strip_tags(md5($_POST['passwordNuevo']))));//obtenemos el nuevo password
	$update="UPDATE usuario SET pass='".$passwordNuevo."' WHERE userid='".$userActivo."'";//preparamos el update
	$queryUpdate=mysql_query($update,$conexion);//realizamos el update
