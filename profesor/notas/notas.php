...
<?php
	...
	$users=mysql_query($select_user,$conexion);
		// WHILE para recorrer todos los usuarios de la BBDD.
		while($alumno=mysql_fetch_array($users)){
			...
			// Lanzamos consulta.
			$user_notas=mysql_query($select_notas_user,$conexion);
			// Recorremos los datos devueltos.
			$notas=mysql_fetch_array($user_notas);
